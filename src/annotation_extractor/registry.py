...
def get_backend(
    ...
    if db_path:
        # If a path is given but no backend name, try each backend
        # to see which one can handle it. Fall back to the first backend.
    ...
    if not results:
        raise RuntimeError(
            ...
            "set KOBO_DB_PATH, or provide backend_name and db_path."
        )
    if len(results) > 1:
